...
function createPlayerState() {
  ...
  return {
    ...
    hasItemInInventory: (u: string, itemId: string) => (inventories.get(u)?.get(itemId) ?? 0) > 0,
...
function setupCounteredTrade(
  ...
  const proposed = system.proposeTrade('alice', 'bob', itemA,
  ...
  const countered = system.counterTrade('bob', itemB,
...
describe('TradeSystem', () => {
  ...
  describe('confirmTrade', () => {
    ...
    it('allows confirm when both sides trade the same item (net zero, no stack issue)', () => {
      ...
      system.proposeTrade('alice', 'bob', 'pelt',
        state.hasItemInInventory, state.areSameTile, state.isBlocked);
      system.counterTrade('bob', 'pelt', state.hasItemInInventory, state.areSameTile);
