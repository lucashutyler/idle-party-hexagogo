...
- **Game content**: `ContentStore` reads/writes `data/monsters.json`, `data/items.json`, `data/zones.json`, `data/world.json`, `data/sets.json`, `data/shops.json`, `data/tile-types.json`, `data/npcs.json`, `data/quests.json`, `data/dungeons.json`. Auto-seeds from `SEED_*` constants if files missing (NPCs only seed when `NODE_ENV !== 'production'`; quests are never seeded).
