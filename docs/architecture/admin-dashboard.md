...
- **Maps** (`MapsTab`): table of every map (name, id, room count, a **Default** badge on the spawn map) with **+ New Map** (name + auto-derived id), **Rename**, and **Del** (blocked on the default map, maps with rooms, or maps with inbound transitions). Backed by `POST/DELETE /api/admin/world/map` (`?versionId=` for drafts). A rename keeps the map's existing start tile. Editing actions are disabled-with-tooltip outside a draft.
