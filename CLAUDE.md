...
│           └── TradeSystem.ts   # Peer-to-peer item trading (propose/counter/confirm/cancel)
└── tests/
...
- **Class system**: 5 playable classes designed to be weak solo, strong together. `CharacterStats.ts` defines `ClassDefinition` with `baseHp`, `hpPerLevel`, `baseDamage`, `damagePerLevel`, and `damageType`. No stats (STR/INT/etc.) — HP and damage scale linearly per class. Classes: **Knight** (50 HP +5/lvl, 1 dmg, physical), **Archer** (8 HP +1/lvl, 15 dmg, physical), **Priest** (20 HP +2/lvl, 3 dmg, holy), **Mage** (8 HP +1/lvl, 15 dmg, magical), **Bard** (10 HP +1/lvl, 1 dmg, physical). Legacy `Adventurer` class is invalid — existing saves get reset, forcing class selection. Each class has a **skill tree** (`SkillTypes.ts`) with 2+ skills — a starting passive (auto-unlocked free) and an active unlocked with skill points. 3 equip slots: passive@Lv1, active@Lv5, passive@Lv10. Skill points earned every 5 levels (`LEVELS_PER_SKILL_POINT = 5`). Class selection screen shows baseHp, damage, damageType, and starting skill.
- **Damage types**: `DamageType = 'physical' | 'magical' | 'holy'`. Monsters and player classes each have a damage type. Knight Guard passive reduces physical damage to the target only. Priest Bless passive reduces magical/holy damage party-wide. Equipment damage reduction applies to physical damage only. Holy damage is treated as magical for reduction purposes.
...
- **User popup menu**: A contextual popup shown when clicking any username across the app (Users tab, Guild/Party members, TileInfoModal players, chat sender names). Shows: Chat (DM), Guild Invite / "In Guild", Add Friend / Accept / Decline / Revoke / "Friends", Party Invite / "In Party" / "Different Room", Trade (same room + level 5+ only), Block / Unblock. Dismissed on outside click or after action. Implemented as a positioned absolute div in `SocialScreen`.
- **Item trading**: Player-to-player item trading via `TradeSystem` (`server/src/game/social/TradeSystem.ts`). Trade lifecycle: `pending` (initiator proposed with item) → `countered` (target locked in their item) → `confirmed` (initiator approved, caller executes swap) / `cancelled`. Rules: both players same tile, level 5+, one unequipped inventory item each, one active trade per player, blocked users cannot trade. Trades auto-cancel on movement (via `onMembersMoved` callback in `PartyBattleManager`) or disconnect (via `PlayerManager.removeConnection`). Trade state is NOT persisted across server restarts (intentionally ephemeral). `TradeSystem` is pure stateful logic; `PlayerManager` wires all I/O. Client trade UI is a modal overlay opened from the user popup menu — item picker, offer cards with rarity colors and effect text, confirm/cancel buttons. Badge on Users sub-tab when a trade requires attention.
