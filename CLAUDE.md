...
**Admin dashboard (World Manager) features are not player-visible** — admin-only changes get no patch notes entry and no `GAME_VERSION` bump. A PR that mixes admin tooling with player-facing changes only lists the player-facing parts.
...
`data/` is gitignored — it is runtime state (game-state saves) plus operator-uploaded artwork. **Artwork that ships with the game** goes in `assets/<kind>/` instead, which is committed and mounted after the matching `data/` mount, so operator uploads still override the shipped default. Before adding third-party art to `assets/`, confirm its licence permits redistribution — a public repo redistributes everything in it.
