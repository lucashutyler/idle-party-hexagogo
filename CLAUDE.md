...
│   │   ├── NpcTypes.ts        # NPC types, dev-only seed data (SEED_NPCS)
│   │   └── QuestTypes.ts      # Quest types, status state machine, accept/progress helpers
...
│   │   │   ├── NpcsTab.ts            # Modal form for NPCs (id, name, emoji, greeting, questIds)
│   │   │   ├── QuestsTab.ts          # Modal form for quests (objectives, rewards, scope, prereqs, repeat)
...
│   │   └── NpcTalkPopup.ts    # NPC talk popup (portrait + greeting; placeholder for quest UI)
│   └── styles/
...
│       ├── QuestSystem.ts     # Per-player quest state, accept/progress/turn-in, status state machine
│       └── social/
...
├── npcs.json                  # NPC definitions (loaded by ContentStore; dev-seeded only — prod boots empty)
├── quests.json                # Quest definitions (loaded by ContentStore, NOT seeded)
