...
client/                        @idle-party-rpg/client — vanilla TS web client (HTML5 Canvas world map; Phaser fully removed from runtime, package retained until cleanup)
├── src/
...
│   │   ├── CombatScreen.ts    # Primary screen — combatant cards (image + name + HP), monster popup, per-zone combat backgrounds
│   │   ├── MapScreen.ts       # Wraps CanvasWorldMap (HTML5 Canvas, no Phaser); zoom controls, RoomView modal
│   │   ├── CharItemsScreen.ts # Merged Char + Items screen — single scrollable column with hero card, skill loadout, condensed stat card, inventory grid
│   │   ├── SocialScreen.ts    # Social tab — sub-tabs: Party (default), Guild, Leaderboard. Chat moved to a global pop-out
...
- **Multi-screen app shell**: DOM-based screen switching. `ScreenManager` handles show/hide with `onActivate`/`onDeactivate` lifecycle. Combat is the default screen; Map lazy-creates the Canvas world map on first visit. A persistent XP bar sits directly above the bottom nav, visible on every game screen. The bottom nav has six tabs: Combat, Map, Inventory (merged Char+Items), Social, Chat (overlay-mode — toggles a global pop-out instead of swapping screens), Settings.
...
- **Canvas world map**: The world map is rendered with `client/src/ui/CanvasWorldMap.ts` (HTML5 Canvas). It draws a parchment background, hex tiles with drop shadows, zone borders/overlays, the path tween, the player party, and per-tile flags for other parties present. Pan via mouse drag / touch drag, zoom via wheel / pinch / on-screen +/− buttons. Smarter default zoom targets ≥15 tiles along the shorter dimension. Scroll bounce-back lets the user overdrag ~25% then springs back. The map pauses its render loop when the screen is inactive. Phaser (`WorldMapScene.ts` and `Party.ts`) is no longer imported but remains in the repo until cleanup.
...
- **Character & leveling**: Each player has a `CharacterState` (className, level, xp, gold, inventory, equipment, skillLoadout). XP is earned on victory. XP to next level = `floor(18000 * L^1.2 * 1.06^L)`. Max HP = `baseHp + (level-1) * hpPerLevel` via `calculateMaxHp(level, className)`. Base damage = `baseDamage + (level-1) * damagePerLevel` via `calculateBaseDamage(level, className)`. **Skills auto-unlock** at their level milestone — there is no skill-point currency. The first passive is unlocked at Lv1; subsequent skills unlock every `LEVELS_PER_SKILL_POINT` (5) levels via `getUnlockedSkillsForLevel(className, level)`. Equipping (5 slots) is the only constraint.
...
- **Other players on map**: Each state message includes `otherPlayers: { username, col, row, zone, className? }[]`. `CanvasWorldMap` renders party flags per occupied tile in the same zone (deterministic color hash so distinct parties read distinctly), and a "+N" badge on the player's own tile so other-room players are not hidden behind the party bubble. Positions update on each player's own battle cycle.
- **Room view (replaces TileInfoModal)**: Clicking a tile opens `RoomView` (`client/src/ui/RoomView.ts`) with three states. **Current room** (you're here): near-full-screen view, background image (`/room-bg-artwork/{zoneId}-{col}-{row}.png` with `/room-bg-artwork/{zoneId}.png` fallback), parties grouped (your party + other parties), shop button when applicable, click any player to open the user popup. **Remote room (discovered)**: smaller centered popup with name/type/player count and a "Go to room" button. **Undiscovered**: same small popup with an "unexplored" hint. Travelling from a remote-room view to your party arriving at that tile triggers an arrival expand animation.
- **Zoom controls**: Mobile-friendly +/- zoom buttons on the map screen, wired to `CanvasWorldMap.adjustZoom()`.
- **Combat cards**: `CombatScreen` renders each combatant as a small card — portrait image (top), name (truncated for players, multi-line allowed for monsters with longer names), and an HP bar with numeric overlay. Class portraits load from `/class-artwork/{class}.png` with a class-icon emoji fallback if missing; monster art loads from `/monster-artwork/{slug}.png` with a placehold.co fallback. Player cards highlight the current user in gold; dead combatants dim; stunned combatants show a "💫" badge. Cards arrange on the 3x3 grid via CSS-grid mapping their `gridPosition`. Clicking a player opens the user popup; clicking a monster opens the monster popup (name + image only — drops/abilities/resistances stay hidden by design until a future system reveals them).
- **Combat backgrounds**: Each combat stage has a background image — CSS `background-image` chain `/combat-bg-artwork/{zoneSlug}-{col}-{row}.png` → `/combat-bg-artwork/{zoneSlug}.png` → placehold.co fallback. The background is dimmed and scrim'd so cards remain readable.
...
- **Social system**: 3 sub-tabs (Chat moved to a global pop-out — see "Chat pop-out" below):
  - **Party** (default): Every player is always in a party (solo party auto-created, max 5 members). Three-tier role hierarchy: owner > leader > member. Party creator is owner. Owner can promote/demote leaders, transfer ownership, and kick anyone. Leaders can promote members to leader, kick (including other leaders, but not owner), and move the party. Members cannot invite, kick, or move. Pending invite flow: owner/leader invites → target sees pending invite with accept/decline → same-room validated on both invite and accept. Invites auto-expire when either the inviter or invitee moves to a different room. Badge indicator on Party tab when invites pending. 3x3 grid positioning for combat formation. Combat is shared. Leaving/kicked auto-creates new solo party at current position. Party events post personalized chat announcements via `PlayerManager.broadcastPartyEvent`.
  ...
  - **Leaderboard** (was Users): All registered players sorted by level descending by default (proxy for XP — server pre-orders by raw XP when wired up). Sort cycler: Top → Status → A-Z. Each row shows class icon, name, online dot, level badge. Search + filter chips (all / room / zone / friends / guild). Incoming friend requests still appear as a top section. Click any username for the user popup menu (Chat/DM, Friend request, Party invite, Trade, Block, etc.). Data sourced from `ClientSocialState.allPlayers` (`PlayerListEntry` includes `username`, `className?`, `level?`).
- **Chat pop-out**: The Chat sub-tab is gone. Chat is now a global overlay (`client/src/ui/ChatPopout.ts`) toggled from a dedicated **Chat** tab in the bottom nav (`mode: 'overlay'`). Desktop: floating, draggable, freely resizable; clamped to the viewport; geometry persisted to `localStorage`. Mobile: full-screen or fixed bottom-sheet (toggle via the popup's layout button; preference persisted). Filters per channel (color-coded), unified timeline with timestamps. Sends use the existing protocol (`gameClient.sendChat`). Unread state lights up the **Chat** nav tab badge.
...
- **Social badges**: Badge dot (red) on Social bottom-nav tab when there are pending friend requests or party invites. The Chat bottom-nav tab gets its own unread badge driven by `ChatPopout`. Sub-tab badges remain: Leaderboard (incoming friend requests or trade requiring attention) and Party (pending invites).
...
- **Separation of concerns**: HTML5 Canvas for the world map; HTML/CSS for all other UI (camera-independent); pure logic in shared systems
- **A* pathfinding**: Hex distance heuristic with cross-track tie-breaker
- **Visual style**: Pixel/retro RPG — Silkscreen body font + Pixelify Sans display font, CSS custom properties for theming, CSS keyframe animations for battle states. All UI is vanilla HTML/CSS (no framework).
- **Image-everywhere convention**: `client/src/ui/assets.ts` exposes `artworkUrl(kind, id)`, `placeholderUrl(name)`, and `renderAssetImg(kind, id, opts)`. The convention is `/<kind>-artwork/{id}.png` (mirroring the existing `/item-artwork/`); when art is missing, the loader falls through to `placehold.co` and finally to the surrounding background color so layouts always have shape. Kinds in use: `item`, `monster`, `class`, `tile`, `tile-type`, `zone`, `set`, `shop`, `logo`, `parchment`, `combat-bg`, `room-bg`. Each new kind requires an Express static mount in `server/src/index.ts` (only `/item-artwork` is mounted today).
- **Persistent XP bar**: `client/src/ui/PersistentXpBar.ts` renders a thin strip directly above the bottom nav (not inside it) — visible on every game screen. Level badge on the left, fill across; raw XP numbers stay on the merged Inventory screen.
- **State persistence**: Player state is periodically saved (every 30s) and on graceful shutdown via `GameStateStore` interface. Current implementation uses JSON files on disk (`JsonFileStore`). On restore, battle timers start fresh (no retroactive simulation); a "Server back online" log entry is added. On shutdown, a "Server shutting down" log entry is added. Saved state per player: `username`, `battleCount`, `combatLog` (last 1000 entries), `unlockedKeys`, `position`, `target`, `movementQueue`, `character` (className, level, xp, inventory, equipment, skillLoadout — `skillPoints` removed; the field is still tolerated on read for back-compat with old saves and silently dropped), `friends`, `outgoingFriendRequests`, `blockedUsers`, `guildId`, `partyId`, `partyRole`, `partyGridPosition`, `chatHistory` (last 1000 messages), `chatSendChannel`, `chatDmTarget`, `mailbox` (pending gift entries). The `character` field is optional in `PlayerSaveData` — old saves or saves with invalid/legacy classes get `character = null` on load, forcing class re-selection. The `inventory` and `equipment` fields are optional within `character` — old saves default to empty inventory and all-null equipment. Social fields are optional — old saves default to empty. Party state (`partyId`, `partyRole`, `partyGridPosition`) is saved and restored — multi-player parties survive server restarts. Guild data is saved separately in `data/guilds.json`. Active async trades are saved separately in `data/trades.json` via `TradeStore`. Game content is saved separately in `data/monsters.json`, `data/items.json`, `data/zones.json`, `data/world.json` via `ContentStore`. The store interface is swappable for SQLite/Postgres.
