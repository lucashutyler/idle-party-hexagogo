export const PATCH_NOTES: { version: string; notes: string[] }[] = [
  {
    ...
    notes: [
      'World Manager: full admin UI overhaul — utilitarian theme, mobile-friendly with a pop-out sidebar, and a UI Size selector (S/M/L/XL) in the sidebar that also tightens paddings/gaps, not just font size',
      'Admin status bar at the top of every page lets you switch versions and Publish / Deploy / + New Draft from anywhere — no need to bounce to the Versions tab. Creating a new draft auto-selects it so the UI immediately becomes editable',
      ...
      'Tile Types tab shows real hex previews of each tile; non-traversable types get a red hex ring; IDs are hidden and auto-generated; the hex color code only appears when you focus the color picker',
      'Shop edit modal alphabetizes items and adds a live search filter plus a "Show only stocked" toggle',
