...
describe('equipItemForceDestroy', () => {
  it('equips item and destroys old equipped item', () => {
    ...
    // But we have a janky_helmet we could equip in a different slot — let's test with the hand slot
    // Put a different hand item in inventory
    const items: Record<string, ItemDefinition> = {
      ...SEED_ITEMS,
      sharp_sword: { id: 'sharp_sword', name: 'Sharp Sword', rarity: 'common', equipSlot: 'hand' as EquipSlot },
    };
    inv['sharp_sword'] = 1;
    ...
    expect(equip.hand).toBe('sharp_sword');
  ...
  it('equips item when slot is empty', () => {
    ...
    expect(equip.hand).toBe('rusty_dagger');
    expect(inv['rusty_dagger']).toBeUndefined();
  });
